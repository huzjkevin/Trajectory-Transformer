    ...
    while epoch < args.max_epoch:
        ...
        for id_b, batch in enumerate(tr_dl):
            ...
            inp = torch.tensor(
                ...
                .argmin(axis=1)
                .reshape(n_in_batch, -1)
            ).to(device)
            speeds_trg = batch["trg"][:, :, 2:4] * scale
            target = torch.tensor(
                scipy.spatial.distance.cdist(speeds_trg.reshape(-1, 2), clusters)
                .argmin(axis=1)
                .reshape(n_in_batch, -1)
            ).to(device)
            src_att = torch.ones((inp.shape[0], 1, inp.shape[1])).to(device)
            trg_att = (
                subsequent_mask(target.shape[1]).repeat(n_in_batch, 1, 1).to(device)
            )
            start_of_seq = (
                torch.tensor([clusters.shape[0]])
                .repeat(n_in_batch)
                .unsqueeze(1)
                .to(device)
            )
            ...
            loss = F.cross_entropy(
                out.view(-1, out.shape[-1]), target.view(-1), reduction="mean"
            )
            loss.backward()
            optim.step()
            print(
        ...
        with torch.no_grad():
            ...
            for batch in val_dl:
                ...
                inp = torch.tensor(
                    scipy.spatial.distance.cdist(
                        speeds_inp.contiguous().reshape(-1, 2), clusters
                    )
                    .argmin(axis=1)
                    .reshape(n_in_batch, -1)
                ).to(device)
                ...
                target = torch.tensor(
                    scipy.spatial.distance.cdist(
                        speeds_trg.contiguous().reshape(-1, 2), clusters
                    )
                    .argmin(axis=1)
                    .reshape(n_in_batch, -1)
                ).to(device)
                ...
                loss = F.cross_entropy(
                    out.contiguous().view(-1, out.shape[-1]),
            ...
            if args.evaluate:
                ...
                for batch in test_dl:
                    ...
                    inp = torch.tensor(
                        scipy.spatial.distance.cdist(
                            speeds_inp.reshape(-1, 2), clusters
                        )
                        .argmin(axis=1)
                        .reshape(n_in_batch, -1)
                    ).to(device)
                    src_att = torch.ones((inp.shape[0], 1, inp.shape[1])).to(device)
                    trg_att = (
                        subsequent_mask(target.shape[1])
                        .repeat(n_in_batch, 1, 1)
                        .to(device)
                    )
                    start_of_seq = (
                        torch.tensor([clusters.shape[0]])
                    ...
                    preds_tr_b = (
                        clusters[dec_inp[:, 1:].cpu().numpy()].cumsum(1)
